...
import com.noctambulist.foody.FoodyApplication
...
class UserAccountViewModel @Inject constructor(
    ...
    fun updateUserInfo(user: User, imageUri: Uri?) {
        val inputValidation = validateEmail(user.email) is RegisterValidation.Success
        ...
        viewModelScope.launch {
            try {
                val imageBitmap = MediaStore.Images.Media.getBitmap(
                    getApplication<FoodyApplication>().contentResolver,
                    imageUri
                )
                ...
                val result = imageDictionary.putBytes(imageByteArray).await()
